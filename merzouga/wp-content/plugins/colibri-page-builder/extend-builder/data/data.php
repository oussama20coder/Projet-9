...
function get_current_data($post_id = -1, $theme_only = false)
{
    ...
    $is_customizer = colibri_user_can_customize() && (is_customize_page() || is_customize_preview()) && $post_id != -1 && !$theme_only;
    if ($is_customizer) {
    ...
    if (!empty($theme_mods_data)) {
        ...
        $draft_rules = array_get_value($data, "options.sheet_rules", "{}");
        $save_rules = get_plugin_option(ColibriOptionsIds::RULES, "[]");
        $merged_rules = merge_values_by_id($save_rules, $draft_rules, false);
        array_set_value($data, "options.sheet_rules", $merged_rules);
    } else {
